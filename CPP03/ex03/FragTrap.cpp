...
/*   Updated: 2022/02/11 01:11:48 by tkim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (this == &frag) //자기 자신을 대입하는 경우
		return *this;
	this->name = frag.getName();
	this->hit_points = frag.get_HP();
	this->energy_points = frag.get_EP();
	this->attack_damage = frag.get_ATT_DMG();
...
{
	std::cout << "FragTrap " << name << "이 " << target << "을 "<< this->get_ATT_DMG() << " 데미지로 공격했다." << std::endl;
}
